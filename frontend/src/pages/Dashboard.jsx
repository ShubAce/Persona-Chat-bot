...
import { MagnifyingGlassIcon, UserCircleIcon, ChatBubbleLeftRightIcon, ClockIcon, ArrowRightOnRectangleIcon } from "@heroicons/react/24/outline";
...
const Dashboard = () => {
	...
	const popularPersonas = personas
		.filter((persona) =>
			["Albert Einstein", "William Shakespeare", "Leonardo da Vinci", "Marie Curie", "Mahatma Gandhi", "Nikola Tesla"].includes(persona.name)
		)
		.slice(0, 6);
		...
		return (
			<div className="min-h-screen flex items-center justify-center bg-gray-50">
				<div className="text-center">
					...
					<p className="mt-4 text-gray-600">Loading historical figures...</p>
				</div>
			</div>
		);
	}
		...
		return (
			<div className="min-h-screen flex items-center justify-center bg-gray-50">
				<div className="text-center">
					...
					<h2 className="text-2xl font-bold text-gray-900 mb-2">Unable to Load Personas</h2>
					<p className="text-gray-600 mb-4">Please check if the backend server is running on port 8000.</p>
					<button
	...
	return (
		<div className="min-h-screen bg-gray-50">
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="flex justify-between items-center h-16">
						<div className="flex items-center">
							...
							<h1 className="text-xl font-bold text-gray-900">Persona Chat - College Project</h1>
						</div>
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
									...
									>
										<div className="flex items-center justify-between">
											<div className="flex-1 min-w-0">
												<h3 className="font-medium text-gray-900 truncate">{session.persona?.name || "Unknown Persona"}</h3>
												<p className="text-sm text-gray-600 truncate">{session.title || "Untitled Chat"}</p>
												...
												>
													<div className="flex items-center justify-between">
														...
														<button
															...
															className="ml-3 px-3 py-1 text-xs bg-primary-600 text-white rounded-md hover:bg-primary-700 transition-colors"
														>
															Chat Now
														</button>
													</div>
				...
				{/* Personas Grid */}
				<div className="mb-8">
					<h2 className="text-lg font-semibold text-gray-900 mb-4">Choose a Historical Figure ({filteredPersonas.length} available)</h2>
							...
							{filteredPersonas.map((persona) => (
								...
								>
									...
									<div className="space-y-2">
										<h3 className="font-semibold text-gray-900 group-hover:text-primary-600 transition-colors">{persona.name}</h3>
										...
										{persona.nationality && <p className="text-sm text-gray-500">{persona.nationality}</p>}
